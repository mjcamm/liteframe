...
assert(_lf_auth_verify_password('secret123', $raw->password) === true);
assert(_lf_auth_verify_password('wrong', $raw->password) === false);
echo "[PASS] Password verification works\n";

// Test 4: Load user by email (internal, with password)
$loaded = _lf_auth_load_user_by_email('alice@test.com');
assert($loaded !== null);
...
$expired = _lf_jwt_encode(['sub' => 1, 'role' => 'admin', 'exp' => time() - 100], $secret);
assert(_lf_auth_validate_token($expired) === null);
echo "[PASS] Expired token rejected\n";

// Test 8: Tampered token fails
$tampered = $token . 'x';
assert(_lf_auth_validate_token($tampered) === null);
echo "[PASS] Tampered token rejected\n";

// Test 9: Generate refresh token
$refresh = _lf_auth_refresh_token($user);
...
$stored = _lf_auth_validate_refresh($refresh);
assert($stored !== null);
assert($stored->user_id === $user->id);
echo "[PASS] Refresh token validates\n";

// Test 11: Revoke refresh token
_lf_auth_revoke_refresh($refresh);
assert(_lf_auth_validate_refresh($refresh) === null);
