...
TestLF::call('schema_sync', $db2, $TYPES);
...
// --- Test 3: Remove field from config — column stays, hidden from LF::entity_columns() ---
...
TestLF::call('schema_sync', $db3, $TYPES_full);
...
// But LF::entity_columns() should NOT include it
$columnList = LF::entity_columns('article');
assert(!str_contains($columnList, 'body'), 'body should not appear in LF::entity_columns');
assert(str_contains($columnList, 'title'), 'title should appear in LF::entity_columns');
echo "[PASS] Removed field — column stays in DB, hidden from LF::entity_columns()\n";
...
TestLF::call('schema_sync', $db4, $TYPES);
...
TestLF::call('schema_sync', $db5, $TYPES);
...
assert($stored->value === TestLF::call('schema_fingerprint', $TYPES), 'Fingerprint should match');
...
TestLF::call('schema_sync', $db6, $TYPES);
...
TestLF::call('schema_sync', $db7, $TYPES);
...
// --- Test 8: LF::entity_columns fallback for unknown type ---

TestLF::set('types', []);
$result = LF::entity_columns('nonexistent');
assert($result === '*', 'Unknown type should fall back to *');
echo "[PASS] LF::entity_columns falls back to * for unknown type\n";
