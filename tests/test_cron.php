...
// Test 1: cron_run with empty $CRONS does nothing
global $CRONS;
$CRONS = [];
cron_run();
echo "[PASS] cron_run with empty config does nothing\n";

// Test 2: cron_run_task executes function and sets last_run
$CRONS = ['test_inc' => ['function' => 'cron_test_increment', 'every' => 5]];
$ok = cron_run_task('test_inc', $CRONS['test_inc']);
assert($ok === true, 'cron_run_task should return true on success');
...
// Test 3: cron_run_task clears previous error on success
variable_set('cron_test_inc_error', 'old error');
cron_run_task('test_inc', $CRONS['test_inc']);
...
// Test 4: cron_run_task catches exception and stores error
$CRONS['test_fail'] = ['function' => 'cron_test_fail', 'every' => 5];
$ok = cron_run_task('test_fail', $CRONS['test_fail']);
...
// Test 5: cron_run_task with missing function stores error
$CRONS['test_missing'] = ['function' => 'nonexistent_cron_function', 'every' => 5];
$ok = cron_run_task('test_missing', $CRONS['test_missing']);
...
// Test 6: cron_run skips tasks that are not yet due
...
// Test 7: cron_run executes tasks that are due
variable_set('cron_test_count', 0);
...
// Test 8: cron_run executes tasks that have never run (last_run = 0)
variable_set('cron_test_count', 0);
...
// Test 9: cron_run_all runs all tasks ignoring timing
variable_set('cron_test_count', 0);
$CRONS = [
...
// Test 10: cron_run_all with empty config returns empty array
$CRONS = [];
$results = cron_run_all();
