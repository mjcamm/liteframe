...
// --- Test 3: Remove field from config — column stays, hidden from entity_columns() ---
...
// But entity_columns() should NOT include it
$columnList = entity_columns('article');
assert(!str_contains($columnList, 'body'), 'body should not appear in entity_columns');
assert(str_contains($columnList, 'title'), 'title should appear in entity_columns');
echo "[PASS] Removed field — column stays in DB, hidden from entity_columns()\n";
...
schema_sync($db5, $TYPES);

// Fingerprint should be stored
$stored = $db5->one("SELECT value FROM _config WHERE key = 'schema_fingerprint'");
assert($stored !== null, 'Fingerprint should be stored');
assert($stored->value === schema_fingerprint($TYPES), 'Fingerprint should match');
...
// --- Test 8: entity_columns fallback for unknown type ---

$TYPES = [];
$result = entity_columns('nonexistent');
assert($result === '*', 'Unknown type should fall back to *');
echo "[PASS] entity_columns falls back to * for unknown type\n";
