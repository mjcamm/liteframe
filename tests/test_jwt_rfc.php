...
$key = _lf_base64url_decode($jwk_k);
...
assert(
    _lf_base64url_encode($rfc_header_json) === $rfc_header_b64,
...
assert(
    _lf_base64url_encode($rfc_payload_json) === $rfc_payload_b64,
...
assert(_lf_base64url_decode($rfc_header_b64) === $rfc_header_json);
assert(_lf_base64url_decode($rfc_payload_b64) === $rfc_payload_json);
...
$computed_signature = _lf_base64url_encode(
...
[$h, $p, $s] = $parts;
$expected_sig = _lf_base64url_encode(hash_hmac('sha256', "{$h}.{$p}", $key, true));
assert(hash_equals($expected_sig, $s), 'Signature verification of RFC token should pass');
$decoded_payload = json_decode(_lf_base64url_decode($p), true);
...
$result = _lf_jwt_decode($rfc_compact, $key);
...
$my_token = _lf_jwt_encode($my_payload, $key);
$decoded = _lf_jwt_decode($my_token, $key);
...
assert(_lf_jwt_decode($my_token, $wrong_key) === null, 'Wrong key should fail');
...
$our_header_b64 = _lf_base64url_encode($our_header);
assert(
    $our_header_b64 !== $rfc_header_b64,
    'Our compact header differs from RFC whitespace header (expected)'
);
$our_header_decoded = json_decode(_lf_base64url_decode($our_header_b64), true);
$rfc_header_decoded = json_decode(_lf_base64url_decode($rfc_header_b64), true);
