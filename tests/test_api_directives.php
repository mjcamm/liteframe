...
$_SERVER['REQUEST_URI'] = '/';
TestLF::set('request', new Request());

// --- Test 1: Parse $api() directives from types.yml ---

$TYPES = TestLF::call('parse_types', __DIR__ . '/fixtures/types_api.yml');
TestLF::set('types', $TYPES);

$apiDirectives = TestLF::get('api_directives');
assert(isset($apiDirectives['article']), 'article should have API directives');
assert(count($apiDirectives['article']) === 5, 'article should have 5 API actions');
assert($apiDirectives['article']['list'] === 'public');
assert($apiDirectives['article']['view'] === 'public');
assert($apiDirectives['article']['create'] === 'auth');
assert($apiDirectives['article']['update'] === 'auth');
assert($apiDirectives['article']['delete'] === 'admin');
echo '[PASS] Parsed article $api() directives (5 actions)' . "\n";

assert(isset($apiDirectives['page']), 'page should have API directives');
assert(count($apiDirectives['page']) === 2, 'page should have 2 API actions');
assert($apiDirectives['page']['list'] === 'public');
assert($apiDirectives['page']['view'] === 'public');
echo '[PASS] Parsed page $api() directives (2 actions, read-only)' . "\n";

assert(isset($apiDirectives['user']), 'user should have API directives');
assert(count($apiDirectives['user']) === 3, 'user should have 3 API actions parsed');
assert($apiDirectives['user']['create'] === 'public');
assert($apiDirectives['user']['update'] === 'auth');
assert($apiDirectives['user']['delete'] === 'auth');
echo '[PASS] Parsed user $api() directives (3 actions parsed)' . "\n";

assert(!isset($apiDirectives['audit_log']), 'audit_log should have no API directives');
echo '[PASS] Types without $api() have no directives' . "\n";

// --- Test 2: Route generation ---

$routes = TestLF::call('api_routes_from_types');
...
// Create
$article = LF::save('article', ['title' => 'Test Article', 'body' => 'Hello world']);
assert($article->id === 1);
assert($article->title === 'Test Article');
echo "[PASS] LF::save create works for article\n";

$article2 = LF::save('article', ['title' => 'Second Article', 'body' => 'More content']);
assert($article2->id === 2);

// Test list handler
$listResult = TestLF::call('type_handle_list', 'article');
...
echo "[PASS] type_handle_list returns paginated results sorted desc\n";

// Test get handler (need to set up matched_route for route_param)
TestLF::set('matched_route', ['params' => ['id' => '1']]);
$getResult = TestLF::call('type_handle_get', 'article');
assert($getResult->id === 1);
assert($getResult->title === 'Test Article');
echo "[PASS] type_handle_get returns entity by id\n";

// Test get handler with non-existent ID
TestLF::set('matched_route', ['params' => ['id' => '999']]);
$notFound = TestLF::call('type_handle_get', 'article');
assert(isset($notFound['error']), 'Should return error for non-existent entity');
echo "[PASS] type_handle_get returns 404 for non-existent entity\n";

// Test delete handler
TestLF::set('matched_route', ['params' => ['id' => '2']]);
$deleteResult = TestLF::call('type_handle_delete', 'article');
assert($deleteResult['deleted'] === true);
assert(LF::load(2) === null);
echo "[PASS] type_handle_delete removes entity\n";

// Test delete non-existent
TestLF::set('matched_route', ['params' => ['id' => '999']]);
$deleteNotFound = TestLF::call('type_handle_delete', 'article');
assert(isset($deleteNotFound['error']));
echo "[PASS] type_handle_delete returns 404 for non-existent entity\n";

// --- Test 5: Invalid actions are skipped ---

$tmpFile = tempnam(sys_get_temp_dir(), 'lf_test_');
file_put_contents($tmpFile, "test_type:\n  name: string\n  \$api(list): public\n  \$api(patch): public\n  \$api(view): auth\n");
TestLF::call('parse_types', $tmpFile);
$apiDirectives = TestLF::get('api_directives');
assert(isset($apiDirectives['test_type']));
assert(count($apiDirectives['test_type']) === 2, 'Should only have list and view, not patch');
assert(isset($apiDirectives['test_type']['list']));
assert(isset($apiDirectives['test_type']['view']));
assert(!isset($apiDirectives['test_type']['patch']), 'patch should be skipped');
unlink($tmpFile);
...
TestLF::set('db', $db);
TestLF::call('schema_apply', $db, $TYPES);
LF::save('article', ['title' => 'Dispatch Test', 'body' => 'test']);

TestLF::set('matched_route', ['_type' => 'article', '_action' => 'list', 'params' => []]);
$dispatchResult = TestLF::call('type_dispatch', TestLF::get('matched_route'));
assert(isset($dispatchResult['data']));
echo "[PASS] type_dispatch routes to correct handler\n";
...
LF::save('article', ['title' => 'Alpha Article', 'body' => 'first post', 'published' => true]);
LF::save('article', ['title' => 'Beta Article', 'body' => 'second post', 'published' => false]);
LF::save('article', ['title' => 'Gamma Guide', 'body' => 'third post', 'published' => true]);

// Grab actual IDs for comparison/range tests
$allIds = array_map(fn($e) => $e->id, LF::query('article')->sort('id', 'asc')->get());
$firstId = $allIds[0];
$secondId = $allIds[1];
$thirdId = $allIds[2];
...
TestLF::set('request', new Request());
$result = TestLF::call('type_handle_list', 'article');
...
TestLF::set('matched_route', ['auth' => 'public', 'params' => []]);
...
TestLF::set('request', new Request());
$result = TestLF::call('type_handle_create', 'user');
...
TestLF::set('request', new Request());
$injected = TestLF::call('type_handle_create', 'user');
...
TestLF::set('request', new Request());
$dup = TestLF::call('type_handle_create', 'user');
...
TestLF::set('request', new Request());
$noPass = TestLF::call('type_handle_create', 'user');
...
TestLF::set('matched_route', ['auth' => 'admin', 'params' => []]);

$_POST = ['name' => 'Staff User', 'email' => 'staff@example.com', 'password' => 'secret', 'role' => 'editor'];
TestLF::set('request', new Request());
$staff = TestLF::call('type_handle_create', 'user');
...
$testUser = LF::load_by('user', 'email', 'test@example.com');
$testUserId = $testUser->id;
$hackerUser = LF::load_by('user', 'email', 'hacker@example.com');
$hackerUserId = $hackerUser->id;

// Simulate logged-in as testUser
TestLF::set('current_user', $testUser);
TestLF::set('matched_route', ['auth' => 'auth', 'params' => ['id' => (string) $testUserId]]);

// Update own name — should work
$_POST = ['name' => 'Updated Name'];
$_SERVER['REQUEST_METHOD'] = 'PUT';
TestLF::set('request', new Request());
$updated = TestLF::call('type_handle_update', 'user');
...
TestLF::set('request', new Request());
$dupUpdate = TestLF::call('type_handle_update', 'user');
...
$_POST = ['email' => 'newemail@example.com'];
TestLF::set('request', new Request());
$sameEmail = TestLF::call('type_handle_update', 'user');
...
TestLF::set('matched_route', ['auth' => 'auth', 'params' => ['id' => (string) $hackerUserId]]);
$_POST = ['name' => 'Pwned'];
TestLF::set('request', new Request());
$blocked = TestLF::call('type_handle_update', 'user');
...
$_POST = ['role' => 'admin'];
TestLF::set('request', new Request());
$roleUpdate = TestLF::call('type_handle_update', 'user');
...
TestLF::set('matched_route', ['auth' => 'admin', 'params' => ['id' => (string) $hackerUserId]]);
$_POST = ['name' => 'Pwned by Admin'];
TestLF::set('request', new Request());
$blockedAdmin = TestLF::call('type_handle_update', 'user');
...
TestLF::set('matched_route', ['auth' => 'auth', 'params' => []]);
$_POST = ['title' => 'Direct Article', 'body' => 'content'];
$_SERVER['REQUEST_METHOD'] = 'POST';
TestLF::set('request', new Request());
$article = TestLF::call('type_handle_create', 'article');
